...
echo "[1/6] Converting Delphes ROOT files to HDF5..."
if [ ! -f "${BASE_H5}" ]; then
...
echo "[2/6] Inspecting generated HDF5 files..."
python "${INSPECTOR}" --summary "${BASE_H5}"

echo "[3/6] Plotting feature distributions across all samples..."
python "${PLOTTER}" --input-h5 "${BASE_H5}" --output-dir "${PLOTS_DIR}"

# train regression network
echo "[4/6] Training regression network..."
python ${REGRESSION} \
  --input-h5 "${BASE_H5}" \
  --output-dir "$REG_OUTPUT"

echo "[5/6] Applying regression model back to jet_data.h5..."
python ${REG_APPLIER} \
  --input-h5 "${BASE_H5}" \
  --output-h5 "${AUG_H5}" \
...
echo "[6/6] Training classification network..."
python ${CLASSIFIER} \
  --input-h5 "${AUG_H5}" \
