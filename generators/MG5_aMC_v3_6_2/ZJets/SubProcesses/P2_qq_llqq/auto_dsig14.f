...
C       LP=SIGN(1,LPP(IB(2)))
      ...
      CALL RANMAR(RHEL)
      CALL RANMAR(RCOL)
      CALL SMATRIX14(P1,RHEL, RCOL,CHANNEL,1, DSIGUU, SELECTED_HEL(1),
     $  SELECTED_COL(1))
...
C     Apply the bias weight specified in the run card (default is 1.0)
      DSIGUU=DSIGUU*CUSTOM_BIAS(PP,DSIGUU,14,1)

      DSIGUU=DSIGUU*NFACT

      IF (DSIGUU.LT.1D199) THEN
C       Set sign of dsig based on sign of PDF and matrix element
        DSIG14=DSIGN(CONV*PD(0)*DSIGUU,DSIGUU*PD(IPSEL))
      ELSE
...
C       Call UNWGT to unweight and store events
        CALL UNWGT(PP,DSIG14*WGT,14,SELECTED_HEL(1), SELECTED_COL(1),
     $    1)
      ENDIF
...
C     
      ...
      DOUBLE PRECISION FUNCTION DSIG14_VEC(ALL_PP, ALL_XBK, ALL_Q2FACT
...
C     ----------
      ...
      IF(IMODE.EQ.1)THEN
        NFACT = DSIG14(ALL_PP(0,1,1), ALL_WGT(1), IMODE)
        RETURN
      ENDIF
...
C           LP=SIGN(1,LPP(IB(2)))
     ...
     $       ,DSQRT(ALL_Q2FACT(2, IVEC)))
      ...
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=D1(IVEC)*UX2(IVEC)
      ...
      IPROC=IPROC+1  ! d c~ > e+ e- d c~
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=D1(IVEC)*CX2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))
      ...
      IPROC=IPROC+1  ! s u~ > e+ e- s u~
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=S1(IVEC)*UX2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))
      ...
      IPROC=IPROC+1  ! s c~ > e+ e- s c~
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=S1(IVEC)*CX2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))
...
C         Select a flavor combination (need to do here for right sign)
          ...
          CHANNELS(IVEC) = CONFSUB(14,SYMCONF(ICONF_VEC(CURR_WARP)))
          SUBDIAG(14) = CHANNELS(IVEC)  ! only valid if a single process
          CHANNEL = SUBDIAG(14)
     ...
     $       ,1,IVEC))
      ...
      CALL SMATRIX14_MULTI(P_MULTI, HEL_RAND, COL_RAND, CHANNELS,
...
C        1.0)
        DSIGUU=DSIGUU*CUSTOM_BIAS(P1,DSIGUU,14, IVEC)
...
C         Call UNWGT to unweight and store events
          CALL UNWGT(ALL_PP(0,1,IVEC), ALL_OUT(IVEC)*ALL_WGT(IVEC),14,
     $      SELECTED_HEL(IVEC), SELECTED_COL(IVEC), IVEC)
      ...
      SUBROUTINE SMATRIX14_MULTI(P_MULTI, HEL_RAND, COL_RAND, CHANNELS
...
!$OMP DO
      DO IVEC=1, VECSIZE_USED
        CALL SMATRIX14(P_MULTI(0,1,IVEC),
     &	                         hel_rand(IVEC),
     &                           col_rand(IVEC),
     &				 channels(IVEC),
     &                           IVEC,
