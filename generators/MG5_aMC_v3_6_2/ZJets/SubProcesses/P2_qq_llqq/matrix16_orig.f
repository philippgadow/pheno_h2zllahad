...
C      where in initialization mode as well for the helicity.
     ...
     $  THEN
        DO I=1,NCOMB
          IF (GOODHEL(I,16) .OR. NTRY(16)
     $     .LE.MAXTRIES.OR.(ISUM_HEL.NE.0)) THEN
            T=MATRIX16(P ,NHEL(1,I),JC(1),I,AMP2, JAMP2, IVEC)
     ...
     $       .EQ.0.AND.ALLOW_HELICITY_GRID_ENTRIES) THEN
          ...
          ENDIF
        ENDDO
        IF(NTRY(16).EQ.(MAXTRIES+1).AND.DS_GET_DIM_STATUS('Helicity')
     $   .NE.-1) THEN
...
C            update.
     ...
     $              'IMIRROR', IMIRROR
                ENDIF
              ELSE IF (.NOT.GOODHEL(I,16) .AND. (DABS(TS(I)).GT.ANS
     $         *LIMHEL/NCOMB)) THEN
                GOODHEL(I,16)=.TRUE.
                NGOOD = NGOOD +1
                PRINT *,'Added good helicity ',I, 'for process 16'
     $           ,TS(I)*NCOMB/ANS,' in event ',NTRY(16)
              ENDIF
            ENDDO
          ENDIF
          IF(NTRY(16).EQ.MAXTRIES)THEN
...
C           Set right sign for ANS, based on sign of chosen helicity
 ...
 10     CONTINUE
     ...
     $       //' zero but not the total matrix-element'
      ...
      REAL*8 FUNCTION MATRIX16(P,NHEL,IC, IHEL,AMP2, JAMP2, IVEC)
C     
C     Generated by MadGraph5_aMC@NLO v. 3.6.2, 2025-03-19
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     Amplitude(s) for diagram number 4
      ...
      CALL FFV1P0_3(W(1,6),W(1,5),GC_11(IVEC),ZERO, FK_ZERO,W(1,10))
      CALL FFV1_2(W(1,1),W(1,10),GC_11(IVEC),ZERO, FK_ZERO,W(1,6))
C     Amplitude(s) for diagram number 5
      CALL FFV1_0(W(1,6),W(1,2),W(1,8),GC_1,AMP(5))
      CALL FFV1_2(W(1,1),W(1,8),GC_1,ZERO, FK_ZERO,W(1,5))
C     Amplitude(s) for diagram number 6
      CALL FFV1_0(W(1,5),W(1,2),W(1,10),GC_11(IVEC),AMP(6))
C     Amplitude(s) for diagram number 7
      CALL FFV2_3_0(W(1,6),W(1,2),W(1,7),GC_50,GC_58,AMP(7))
      CALL FFV2_3_2(W(1,1),W(1,7),GC_50,GC_58,ZERO, FK_ZERO,W(1,6))
C     Amplitude(s) for diagram number 8
      CALL FFV1_0(W(1,6),W(1,2),W(1,10),GC_11(IVEC),AMP(8))
