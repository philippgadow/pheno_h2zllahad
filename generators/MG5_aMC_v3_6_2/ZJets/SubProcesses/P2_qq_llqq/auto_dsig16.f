...
C       LP=SIGN(1,LPP(IB(2)))
      ...
      CALL RANMAR(RHEL)
      CALL RANMAR(RCOL)
      CALL SMATRIX16(P1,RHEL, RCOL,CHANNEL,1, DSIGUU, SELECTED_HEL(1),
     $  SELECTED_COL(1))
...
C     Apply the bias weight specified in the run card (default is 1.0)
      DSIGUU=DSIGUU*CUSTOM_BIAS(PP,DSIGUU,16,1)

      DSIGUU=DSIGUU*NFACT

      IF (DSIGUU.LT.1D199) THEN
C       Set sign of dsig based on sign of PDF and matrix element
        DSIG16=DSIGN(CONV*PD(0)*DSIGUU,DSIGUU*PD(IPSEL))
      ELSE
...
C       Call UNWGT to unweight and store events
        CALL UNWGT(PP,DSIG16*WGT,16,SELECTED_HEL(1), SELECTED_COL(1),
     $    1)
      ENDIF
...
C     
      ...
      DOUBLE PRECISION FUNCTION DSIG16_VEC(ALL_PP, ALL_XBK, ALL_Q2FACT
...
C     ----------
      ...
      IF(IMODE.EQ.1)THEN
        NFACT = DSIG16(ALL_PP(0,1,1), ALL_WGT(1), IMODE)
        RETURN
      ENDIF
...
C         Select a flavor combination (need to do here for right sign)
          ...
          CHANNELS(IVEC) = CONFSUB(16,SYMCONF(ICONF_VEC(CURR_WARP)))
          SUBDIAG(16) = CHANNELS(IVEC)  ! only valid if a single process
          CHANNEL = SUBDIAG(16)
     ...
     $       ,1,IVEC))
      ...
      CALL SMATRIX16_MULTI(P_MULTI, HEL_RAND, COL_RAND, CHANNELS,
...
C        1.0)
        DSIGUU=DSIGUU*CUSTOM_BIAS(P1,DSIGUU,16, IVEC)
...
C         Call UNWGT to unweight and store events
          CALL UNWGT(ALL_PP(0,1,IVEC), ALL_OUT(IVEC)*ALL_WGT(IVEC),16,
     $      SELECTED_HEL(IVEC), SELECTED_COL(IVEC), IVEC)
      ...
      SUBROUTINE SMATRIX16_MULTI(P_MULTI, HEL_RAND, COL_RAND, CHANNELS
...
!$OMP DO
      DO IVEC=1, VECSIZE_USED
        CALL SMATRIX16(P_MULTI(0,1,IVEC),
     &	                         hel_rand(IVEC),
     &                           col_rand(IVEC),
     &				 channels(IVEC),
     &                           IVEC,
