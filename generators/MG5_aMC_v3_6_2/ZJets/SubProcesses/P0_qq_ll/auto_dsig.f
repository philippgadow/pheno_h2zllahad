...
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
C     
C     Process: u u~ > e+ e- WEIGHTED<=4
C     Process: u u~ > mu+ mu- WEIGHTED<=4
C     Process: c c~ > e+ e- WEIGHTED<=4
C     Process: c c~ > mu+ mu- WEIGHTED<=4
C     Process: d d~ > e+ e- WEIGHTED<=4
C     Process: d d~ > mu+ mu- WEIGHTED<=4
C     Process: s s~ > e+ e- WEIGHTED<=4
C     Process: s s~ > mu+ mu- WEIGHTED<=4
C     
...
C            the warp)
     ...
     $ ,IMIRROR_VEC,VECSIZE_USED)  ! u u~ > e+ e-
      IF(IPROC.EQ.2) CALL DSIG2_VEC(ALL_P1,ALL_XBK,ALL_Q2FACT
     $ ,ALL_CM_RAP,ALL_WGT,IMODE,ALL_OUT,SYMCONF, CONFSUB,ICONF_VEC
     $ ,IMIRROR_VEC,VECSIZE_USED)  ! d d~ > e+ e-
